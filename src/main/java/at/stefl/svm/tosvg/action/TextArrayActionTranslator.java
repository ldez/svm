...
public class TextArrayActionTranslator extends SVGActionTranslator<TextArrayAction> {
    ...
    // TODO: implement dx
    @Override
    protected void translateImpl(final TextArrayAction action, final SVGStateWriter out, final TranslationState state) throws IOException {
        Vector2d point = action.getStartPoint().getAsVector2d();
        final String text = action.getString().substring(action.getIndex(), action.getIndex() + action.getLength());
