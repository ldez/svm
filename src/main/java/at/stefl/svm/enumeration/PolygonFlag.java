...
public enum PolygonFlag {
    ...
    NORMAL(PolygonFlagConstants.POLY_NORMAL), SMOOTH(PolygonFlagConstants.POLY_SMOOTH), CONTROL(PolygonFlagConstants.POLY_CONTROL), SYMMETRY(PolygonFlagConstants.POLY_SYMMTR);

    private static final ObjectTransformer<PolygonFlag, Byte> CODE_KEY_GENERATOR = new ObjectTransformer<PolygonFlag, Byte>() {

        @Override
        public Byte transform(final PolygonFlag value) {
            return value.code;
        }
    };
    ...
    public static PolygonFlag getActionTypeByCode(final int code) {
        return BY_CODE_MAP.get(code);
    }

    private final byte code;

    private PolygonFlag(final byte code) {
        this.code = code;
    }
