...
public enum HatchStyle {
    ...
    SINGLE(HatchStyleConstants.HATCH_SINGLE), DOUBLE(HatchStyleConstants.HATCH_DOUBLE), TRIPLE(HatchStyleConstants.HATCH_TRIPLE), FORCE_EQUAL_SIZE(HatchStyleConstants.HATCH_FORCE_EQUAL_SIZE);

    private static final ObjectTransformer<HatchStyle, Integer> CODE_KEY_GENERATOR = new ObjectTransformer<HatchStyle, Integer>() {

        @Override
        public Integer transform(final HatchStyle value) {
            return value.code;
        }
    };
    ...
    public static HatchStyle getByCode(final int code) {
        return BY_CODE_MAP.get(code);
    }

    private final int code;

    private HatchStyle(final int code) {
        this.code = code;
    }
