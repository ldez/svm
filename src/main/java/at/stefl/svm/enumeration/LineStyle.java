...
public enum LineStyle {
    ...
    NONE(LineStyleConstants.LINE_NONE), SOLID(LineStyleConstants.LINE_SOLID), DASH(LineStyleConstants.LINE_DASH), FORCE_EQUAL_SIZE(LineStyleConstants.LINE_FORCE_EQUAL_SIZE);

    private static final ObjectTransformer<LineStyle, Integer> CODE_KEY_GENERATOR = new ObjectTransformer<LineStyle, Integer>() {

        @Override
        public Integer transform(final LineStyle value) {
            return value.code;
        }
    };
    ...
    public static LineStyle getByCode(final int code) {
        return BY_CODE_MAP.get(code);
    }

    private final int code;

    private LineStyle(final int code) {
        this.code = code;
    }
